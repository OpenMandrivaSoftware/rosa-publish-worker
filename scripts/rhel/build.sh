...
if [ ! -f "$comps_xml" ]; then
  cd /home/vagrant
  curl -k -L -O https://abf.rosalinux.ru/server/comps_xml/archive/comps_xml-$build_for_platform.tar.gz
...
function build_repo {
  ...
  if [ "$regenerate" != 'true' ] ; then
    echo "createrepo_c -v --update -d -g $comps_xml -o $path $path"
    createrepo_c -v --update -d -g "$comps_xml" -o "$path" "$path"
  else
    echo "createrepo_c -v -d -g $comps_xml -o $path $path"
    createrepo_c -v -d -g "$comps_xml" -o "$path" "$path"
  fi
  # Save exit code
